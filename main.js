let startClickSound, preloaderSound, scrollSound, backgroundMusic;
...
document.getElementById("enableBtn").onclick = function () {
  ...
  scrollSound = document.getElementById("scrollSound");
...
function setupSectionScrollSounds() {
  ...
  function isInSectionsArea() {
    if (!footer) return true;
    return window.scrollY < footer.offsetTop - window.innerHeight;
  }
  ...
  window.addEventListener("scroll", () => {
    ...
    clearTimeout(scrollTimeout);
    scrollTimeout = setTimeout(stopScrollSound, 150);
  });
}
...
function startAnimations() {
  ...
  function updateAnimations() {
    ...
    if (progress <= 0.1) {
      awarenessState = `[${freq1}] ОСОЗНАНИЕ: ТИШИНА`;
      becomingState = `.${freq2} СОСТОЯНИЕ: ПУСТОТА`;
      energyState = `{${energy}} ЭНЕРГИЯ: СПЯЩАЯ`;
    } else if (progress <= 0.25) {
      awarenessState = `[${freq1}] ОСОЗНАНИЕ: ЗАРОЖДЕНИЕ`;
      becomingState = `.${freq2} СОСТОЯНИЕ: ПОЯВЛЕНИЕ`;
      energyState = `{${energy}} ЭНЕРГИЯ: ПРОБУЖДЕНИЕ`;
    } else if (progress <= 0.5) {
      awarenessState = `[${freq1}] ОСОЗНАНИЕ: ТЕЧЕНИЕ`;
      becomingState = `.${freq2} СОСТОЯНИЕ: РАСШИРЕНИЕ`;
      energyState = `{${energy}} ЭНЕРГИЯ: НАРАСТАНИЕ`;
    } else if (progress <= 0.75) {
      awarenessState = `[${freq1}] ОСОЗНАНИЕ: ВОСХОЖДЕНИЕ`;
      becomingState = `.${freq2} СОСТОЯНИЕ: РАСТВОРЕНИЕ`;
      energyState = `{${energy}} ЭНЕРГИЯ: ИЗЛУЧЕНИЕ`;
    } else if (progress <= 0.9) {
      awarenessState = `[${freq1}] ОСОЗНАНИЕ: ПРЕВОСХОЖДЕНИЕ`;
      becomingState = `.${freq2} СОСТОЯНИЕ: БЕСКОНЕЧНОСТЬ`;
      energyState = `{${energy}} ЭНЕРГИЯ: ПЕРЕПОЛНЕНИЕ`;
    } else {
      awarenessState = `[${freq1}] ОСОЗНАНИЕ: ЕДИНСТВО`;
      becomingState = `.${freq2} СОСТОЯНИЕ: ВЕЧНОСТЬ`;
      energyState = `{${energy}} ЭНЕРГИЯ: ЧИСТАЯ`;
    }

    const presenceIntensity = Math.max(0, 1 - progress);
    if (presenceIntensity > 0.8) {
      presenceState = `.${presence} ПРИСУТСТВИЕ: ПЛОТНОЕ`;
    } else if (presenceIntensity > 0.6) {
      presenceState = `.${presence} ПРИСУТСТВИЕ: СМЯГЧЕНИЕ`;
    } else if (presenceIntensity > 0.4) {
      presenceState = `.${presence} ПРИСУТСТВИЕ: ПРОЗРАЧНОЕ`;
    } else if (presenceIntensity > 0.2) {
      presenceState = `.${presence} ПРИСУТСТВИЕ: ЭФИРНОЕ`;
    } else {
      presenceState = `.${presence} ПРИСУТСТВИЕ: ПУСТОТА`;
    }
...
function setupInfoPanels() {
  document.querySelectorAll("[data-panel-open]").forEach((link) => {
    link.addEventListener("click", (e) => {
      e.preventDefault();
      const panel = document.getElementById(link.dataset.panelOpen);
      if (!panel) return;
